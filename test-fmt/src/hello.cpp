...
double measurePrintf(int iterations) {
...
double measureStdCout(int iterations) {
    auto start = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iterations; ++i) {
...
double measureFmtPrint(int iterations) {
    auto start = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iterations; ++i) {
        fmt::print("Iteration: {}\n", i);
    }
...
int main() {
    ...
    double fmtprint_time = measureFmtPrint(iterations);
    double stdcout_time = measureStdCout(iterations);
    double print_time = measurePrintf(iterations);
    std::cout << "printf time: " << print_time << " sec\n";
    std::cout << "std::cout time: " << stdcout_time << " sec\n";
    std::cout << "fmt::print time: " << fmtprint_time << " sec\n";
    return 0;
}